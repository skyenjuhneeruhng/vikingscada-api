...
module.exports = {
  ...
  getRoleByType: async (type) => {
    const role = await strapi.query('role', 'users-permissions').findOne({ type: type }, [])
    return role;
  },

  getRole: async (roleID, plugins, clean = false) => {
    const role = await strapi.query('role', 'users-permissions').findOne({ id: roleID }, ['users', 'permissions']);
    ...
    role.permissions = role.permissions.reduce((acc, permission) => {
      if (!clean){
        ...
        if (_.toNumber(permission.enabled) == true)
          _.set(acc, `${permission.type}.controllers.${permission.controller}.${permission.action}`, true);
      }
  ...
  updatePermissions: async function (cb) {
    ...
    if (!_.isEqual(actions, currentActions) || permissions < 1) {
      ...
      const defaultPolicy = (obj, role) => {
        ...
        const isActivateGateway = obj.action === 'activategateway' && role.type === 'public';
        const isCompanyRegister = obj.controller === 'company' && obj.action === 'register' && role.type === 'public';
        const sendVerify = obj.controller === 'company' && obj.action === 'sendverifycode' && role.type === 'public';
        const saveGeolocation = obj.action === 'savegeogateway' && role.type === 'public';
        const saveTrafficlocation = obj.action === 'savetrafficgateway' && role.type === 'public';
        const twillioVoiceCallback = obj.action === 'getalertsforwidget' && role.type === 'public';
        const enabled = saveTrafficlocation || twillioVoiceCallback || saveGeolocation || sendVerify || isCompanyRegister || isActivateGateway || isCallback || isRegister || role.type === 'root' || isInit || isPassword || isNewPassword || isMe || isReload || isConnect;
