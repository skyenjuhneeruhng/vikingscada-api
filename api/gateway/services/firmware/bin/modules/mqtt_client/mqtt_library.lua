...
MQTT.Utility = require("modules.mqtt_client.utility")
...
MQTT.client.SOCKET_TIME_OUT = 10 -- 15 sek ???? ---------------
...
function MQTT.client.create(                                      -- Public API
  ...
  mqtt_client.identifier = nil
  mqtt_client.callback = callback  -- function(topic, payload)
  MQTT.client.on_disconnect = disconnect_callback
...
function MQTT.client:connect(                                     -- Public API
  ...
  if self == nil then
  	 return("MQTT.client:connect(): self is null ")
  end               
  ...
  if (self.socket_client == nil) then
    return("MQTT.client:connect(): Couldn't open socket connection")
  end
  ...
  if next(self.params) == nil then
    ...
    self.socket_client = ssl.wrap(self.socket_client, ctx) -- assert
...
-- modify --------------- ?
  ...
  if(message_type == MQTT.message.TYPE_SUBSCRIBE) then
 	message = string.char(0x82) -- 1000 0010
  else
 	message = string.char(MQTT.Utility.shift_left(message_type, 4))
  end 
 -------
...
function MQTT.client:publish(                                     -- Public API
  ...
  if (self.connected == false) then
    print("MQTT.client:publish(): Not connected")
    return false
  end
  ...
  if self:message_write(MQTT.message.TYPE_PUBLISH, message) ~= nil then --?????
 	  return false
  end
...
function MQTT.client:subscribe(topics)  -- table of strings                   -- Public API
