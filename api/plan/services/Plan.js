...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Plan
      ...
      .populate(_.keys(_.groupBy(_.reject(strapi.models.plan.associations, {autoPopulate: false}), 'alias')).join(' ')).count().then(function(count){
          ...
          return Plan
            ...
            .populate(_.keys(_.groupBy(_.reject(strapi.models.plan.associations, {autoPopulate: false}), 'alias')).join(' ')).exec().then(function(items){
              response.list = items;
              return response;
            });
        });
  ...
  add: async (values) => {
    const query = await Plan.create(_.omit(values, _.keys(_.groupBy(strapi.models.plan.associations, 'alias'))));
    const data = query.toJSON ? query.toJSON() : query;

    await strapi.hook.mongoose.manageRelations('plan', _.merge(data, { values }));
